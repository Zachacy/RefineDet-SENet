...
from layers import *
from data import voc_refinedet, coco_refinedet, kitti_refinedet
import os
...
class RefineDet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        ...
        self.cfg = (coco_refinedet, voc_refinedet, kitti_refinedet)[num_classes == 4]
        ...
        self.tcb0 = nn.ModuleList(TCB[0])# Top of TCB part
        self.tcb1 = nn.ModuleList(TCB[1])# Transport of TCB part
        self.tcb2 = nn.ModuleList(TCB[2])# Bottom of TCB part
        self.fire_block = nn.ModuleList(TCB[3])
        self.squeeze = nn.ModuleList(TCB[4])
        if phase == 'test':
    ...
    def forward(self, x):
        ...
        for k, v in enumerate(sources[::-1]):
            s = v
            for i in range(3):
                s = self.tcb0[(3-k)*3 + i](s)# loop:back to front
            
            ### Fire Block ###
            avg = s.view(s.shape[0],256,s.shape[2]*s.shape[3]).mean(dim = 2)# view(batch,channel, w*h)
            for j in range(0,3):
                avg = self.fire_block[(3-k)*3 + j](avg)
            s *= avg.view(s.shape[0],256,1,1).expand_as(s)
            


            if k != 0:
                avg = p.view(p.shape[0],256,p.shape[2]*p.shape[3]).mean(dim = 2)
                for j in range(0,3):
                    avg = self.fire_block[(3-k)*3 + j](avg)
                u = p*avg.view(p.shape[0],256,1,1).expand_as(p)
                u = self.tcb1[3-k](u)
                s = torch.cat([u+s,s], dim = 1)
                s = self.squeeze[3-k](s)
            for i in range(3):
...
def add_tcb(cfg):
    ...
    feature_pred_layers = []
    features_Fire_layers = []
    feature_squeeze_layers = []
    for k, v in enumerate(cfg):
        feature_scale_layers += [nn.Conv2d(cfg[k], 256, 3, padding=1),
                                 nn.ReLU(inplace=True),
                                 nn.Conv2d(256, 256, 3, padding=1)
        ]# Top of TCB part
        feature_pred_layers += [nn.PReLU(),
                                nn.Conv2d(256, 256, 3, padding=1),
                                nn.ReLU(inplace=True)
        ]
        features_Fire_layers += [nn.Linear(256, 256),
                                 nn.ReLU(),
                                 nn.Sigmoid()]
        # Bottom of TCB part
        #Up-Sampe part
        if k != len(cfg) - 1:
            ...
            feature_squeeze_layers += [nn.Conv2d(512,256,1)]
    return (feature_scale_layers, feature_upsample_layers, feature_pred_layers, features_Fire_layers,feature_squeeze_layers)
...
def build_refinedet(phase, size=320, num_classes=21):
    ...
    TCB_ = add_tcb(tcb[str(size)])# TCB layer[0:3]
